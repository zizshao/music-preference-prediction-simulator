...
Y_tr_class = np.select([Y_tr <= 0.3,
                        (Y_tr > 0.3) & (Y_tr < 0.7),
                        Y_tr >= 0.7],
                       [1,0,-1])
train_set["class"] = Y_tr_class
Y_ts_class = np.select([Y_ts <= 0.3,
                        (Y_ts > 0.3) & (Y_ts < 0.7),
                        Y_ts >= 0.7],
                       [1,0,-1])
test_set["class"] = Y_ts_class

train_stz = stz(train_set, train_set, stz_feat)
class_mean = get_class_mean(train_stz, "class", all_feat)

X_ts_class = stz(test_set, train_set, stz_feat)[all_feat].to_numpy()
pred_class = min_dist_classify(X_ts_class, class_mean)

acc = np.mean(pred_class == Y_ts_class)
print("The accuracy rate for classification:", acc)
